...
import NetixswapV1Pair from '@aidigitaldreamers/v1-core/build/NetixswapV1Pair.json'
...
describe('scenario:FeeTo', () => {
  ...
  beforeEach('deploy netixswap v1', async () => {
    factory = await deployContract(wallet, NetixswapV1Factory, [wallet.address])
  })
  ...
  describe('tokens', () => {
    ...
    beforeEach('make test tokens', async () => {
      const { timestamp: now } = await provider.getBlock('latest')
      const token0 = await deployContract(wallet, Netix, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token0)
      const token1 = await deployContract(wallet, Netix, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token1)
    })
    ...
    beforeEach('create fee liquidity', async () => {
      ...
      pair = new Contract(pairAddress, NetixswapV1Pair.abi).connect(wallet)

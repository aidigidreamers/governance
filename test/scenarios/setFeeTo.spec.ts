...
describe('scenario:setFeeTo', () => {
  ...
  beforeEach('deploy netixswap v1', async () => {
    factory = await deployContract(wallet, NetixswapV1Factory, [timelock.address])
  })
  ...
  it('setFeeTo', async () => {
    ...
    const description = 'Set feeTo on the NetixswapV1Factory to the timelock address.'

    // activate balances
    await netix.delegate(wallet.address)

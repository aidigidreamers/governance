...
export async function governanceFixture(
  ...
  // deploy Netix, sending the total supply to the deployer
  const { timestamp: now } = await provider.getBlock('latest')
  const timelockAddress = Contract.getContractAddress({ from: wallet.address, nonce: 1 })
  const netix = await deployContract(wallet, Netix, [wallet.address, timelockAddress, now + 60 * 60])
  ...
  const governorAlpha = await deployContract(wallet, GovernorAlpha, [timelock.address, netix.address])
  expect(governorAlpha.address).to.be.eq(governorAlphaAddress)

  return { netix, timelock, governorAlpha }
}

...
contract FeeTo {
    ...
    function renounce(address pair) public returns (uint value) {
        ...
        TokenAllowState storage token0AllowState = tokenAllowStates[INetixswapV1Pair(pair).token0()];
        TokenAllowState storage token1AllowState = tokenAllowStates[INetixswapV1Pair(pair).token1()];
        ...
        if (
            ...
            value = INetixswapV1Pair(pair).balanceOf(address(this));
            if (value > 0) {
                ...
                assert(INetixswapV1Pair(pair).transfer(pair, value));
                INetixswapV1Pair(pair).burn(pair);
            }
    ...
    function claim(address pair) public returns (uint value) {
        ...
        if (
            ...
            if (value > 0) {
                ...
                assert(INetixswapV1Pair(pair).transfer(feeRecipient, value));
            }
        }
    }
}

interface INetixswapV1Pair {
